...
class ShipmentProcessor:
    ...
    def process(self,event):
        ...
        same=(current is not None and current.sequence==event.sequence and current.status==event.status
              and current.order_id==event.order_id and current.sku==event.sku)
        if not is_newer(event,current) and not same:
        ...
        if should_reserve(event.status): self.inventory.reserve(event.order_id,event.sku)
        if should_release(event.status): self.inventory.release(event.order_id,event.sku)
        if should_notify(event.status): self.notifications.send(event.event_id,event.shipment_id,event.status.name)
